/*
   ...
   Basic code taken from ros_motor_test, then modified
   Subscribes to motor speed topic, which gives a frequency for pwm signal
   Also publishes current speed
   Meant to interact with IMU:
    This code is uploaded on one Teensy, IMU code on another Teensy.
    Both Teensy nodes setup on ROS, with another node translating imu data (linear acceleration in x-axis) to a motor speed
    This demonstrates sensor control of motor (no encoder currently available to test with)
...
void messageCb( const std_msgs::Int32& motor_speed) {
  int value = digitalRead(13);
  digitalWrite(13, HIGH - value); // blink the led
  motor_vel=motor_speed.data;
  ...
  delay(10);
  analogWriteFrequency(LAnkleFB_B, abs(motor_speed.data));       //Move motor
  delay(10);

}
ros::Subscriber<std_msgs::Int32> sub1("motor_speed", &messageCb );
...
void setup()
{
  ...
  analogWriteFrequency(LAnkleFB_B, 1000);
