/*
    ...
    v2- All Teensys dumps messages on the same topic
...
//For posting finished message, but not used anymore
//void messageCb2( const std_msgs::Bool& bool_msg){
//  Ofinished_flag=bool_msg.data;
////  if (Ofinished_flag) Odump_end=millis();
//}

// Count received messages. With the first message, start time, and end time with last message
...
void setup()
{
  ...
  //Get some parameters from roslaunch file
...
void loop()
{
  ...
  //Odump_begin=millis();
  //Important to call spinOnce() function since that is when program checks for any new messages to respond to
